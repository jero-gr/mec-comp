function [T] = difFinitas(xnode, model, cb, et, T_ini)

  % xnode: vector de coordenadas nodales

  % model: constantes del modelo:
  %   model.rho: densidad del material
  %   model.cp: capacidad calorífica del material
  %   model.k: conductividad térmica del material
  %   model.c: fuente térmica interna o términos de reacción
  %   model.G: fuente de calor en cada nodo del dominio

  % cb: condiciones de borde
  %   cb(1,:): condiciones de borde en x=L1 (extremo izquierdo)
  %   cb(2,:): condiciones de borde en x=L2 (extremo derecho)
  %     cb(:,1): tipo de condición de borde (1: dirichlet, 2: neumann, 3: robin)
  %     cb(:,2): temperatura (dirichlet), flujo (neumann), coeficiente de convección h (robin)
  %     cb(:,3): -1 (dirichlet, neumann), temperatura externa (robin)

  % et: esquema temporal
  %   et(1): tipo de análisis temporal (0: estado estacionario, 1: esquema explícito, 2: esquema implícito)
  %   et(2): numero máximo de iteraciones
  %   et(3): tolerancia
  %   et(4): paso del tiempo
